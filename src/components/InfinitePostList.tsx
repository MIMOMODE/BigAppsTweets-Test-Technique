...
export function InfinitePostList({
  ...
  if (posts == null || posts.length === 0) {
    return (
      <h2 className="my-4 text-center text-2xl text-gray-500">Pas de posts pour le moment :C</h2>
    );
  }
...
function PostCard({
  ...
  const toggleLike = api.timeline.toggleLike.useMutation({
    onSuccess: ({ addedLike }) => {
      const updateData: Parameters<
        typeof trpcUtils.timeline.infiniteFeed.setInfiniteData
      >[1] = (oldData) => {
      ...
      trpcUtils.timeline.infiniteFeed.setInfiniteData({}, updateData);
      trpcUtils.timeline.infiniteFeed.setInfiniteData(
        { onlyFollowing: true },
        updateData
      );
      trpcUtils.timeline.infiniteProfileFeed.setInfiniteData(
        { userId: user.id },
        updateData
      );
    },
